...
\title{Exact weighted Pauli uncertainty from matching polytopes and simplicial quasi-line graphs\thanks{Independent
...
the previously sufficient $h$-perfect route. A centralized ablation over all
1245 nonempty graph-atlas roots through seven vertices and 6225 weighted
instances separates the ingredients: degree constraints are exact on 66.89\%,
cliques plus odd cycles on 84.69\%, and the complete odd-set system on 100\%.
All three are exact on 710 bipartite controls. A symmetric norm-one
contraction violates a triangle odd-set inequality, while its skew control
saturates it. Scaled tests add 180 sparse roots through order 30, four weight
regimes, controlled families through order 132, and a density stress through
order 40. Local relaxations are often nearly exact on random sparse cases but
retain constant gaps on planted blossoms. These computations test the
mechanism; the theorem remains analytic.
Beyond line graphs, we prove that every simplicial claw-free quasi-line graph
is rank-perfect and therefore $\hbar$-perfect. The proof excludes the
clique-circulants responsible for every nonrank quasi-line facet. An exact
nine-vertex witness shows this extension is strict over both line graphs and
$h$-perfect graphs.
\end{abstract}
...
We then combine a simplicial-clique heredity lemma with the modern facet
structure of fuzzy circular interval graphs to prove $\hbar$-perfection for
the strictly larger class of simplicial claw-free quasi-line graphs.
...
\section{Beyond line graphs: the rank-perfect SCF bridge}
...
For completeness, we use the elementary discrete circular-diameter fact that
a pairwise $(p-1)$-close subset $S\subseteq\mathbb Z_n$ which is not contained
in $p$ consecutive residues satisfies
\begin{equation}\label{eq:nonlocal-clique-bound}
 |S|\leq 3p-n.
\end{equation}
Here is a short count. Choose an inclusion-minimal nonlocal subset. The
circular-arc Helly obstruction gives three cyclic extreme points; if their
clockwise gaps are $a,b,c$, then $a+b+c=n$ and each is at most $p-1$.
Assign every point of $S$ to the half-open gap following one of these three
extremes. Pairwise closeness to the other two extremes leaves respectively at
most $p-a$, $p-b$, and $p-c$ admissible integer positions. Summing gives
$(p-a)+(p-b)+(p-c)=3p-n$, proving~\eqref{eq:nonlocal-clique-bound}.
...
$C(n,p)$ is the complement of the odd cycle $C_{2p+1}$; odd antiholes of
length at least seven have no simplicial clique~\cite{csss}. Hence no
nonlocal clique is simplicial either.
\end{proof}
...
reported numerical constant is read from six frozen JSON records by the
