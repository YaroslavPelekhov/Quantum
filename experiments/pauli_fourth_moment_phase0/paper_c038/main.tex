...
saturates it. These computations test the mechanism; the theorem remains
analytic.
\end{abstract}
...
reported numerical constant is read from the two frozen JSON records by the
manuscript checker. The second record includes graph identifiers, all weight
vectors and relaxation values, aggregate summaries, explicit scope flags, and
hashes for the three CSV tables and two figures.
