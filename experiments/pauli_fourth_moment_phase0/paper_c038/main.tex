...
\title{Skew contractions and exact weighted Pauli uncertainty on all line graphs\thanks{Independent
...
Beyond line graphs, we prove the general bridge that every rank-perfect
simplicial claw-free graph is $\hbar$-perfect. Hence every simplicial
claw-free semi-line graph is $\hbar$-perfect. An exact nine-vertex witness
shows this extension is strict over both line graphs and $h$-perfect graphs.
\end{abstract}
...
named class beyond the matching construction.
...
\caption{Rank-perfect boundary audit. The order-nine panel is exhaustive for
the frozen SCF census; the larger proper circular-arc panel is a conditioned
seeded sample, and the web panel is an exact family sweep. These computations
support, but do not prove, the stronger SCF quasi-line conjecture.}
\label{fig:rank-perfect-boundary}
\end{figure}

The tempting statement that all quasi-line graphs are $\hbar$-perfect is
false: the anti-heptagon is quasi-line and is the smallest known
$\hbar$-imperfect graph~\cite{xu}. We therefore record only the conjecture
that every \emph{SCF} quasi-line graph is rank-perfect. Neither that conjecture
nor unrestricted SCF $\hbar$-perfection is used in any theorem here.
...
boundary used only in Theorem~\ref{thm:rank-perfect-scf}'s corollary
\cite{oriolo,eosv}.
...
The rank-perfect SCF bridge also shows that the line-graph mechanism is not the
only analytic route. It extends exact Pauli uncertainty to every SCF semi-line
graph and to a certified graph that is neither line nor $h$-perfect. The
larger quasi-line statement remains a conjecture, explicitly separated from
the proved result.
...
reported numerical constant is read from five frozen JSON records by the
