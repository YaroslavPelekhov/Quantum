...
on these four cases preserve the exact sign, while 72/80 are certified, leaving
eight conservative false negatives of the certificate. The smallest-margin
24-qubit case accounts for
...
Figure~\ref{fig:independent} gives all ten setting/ordering locations,
comprising 20 backend cohorts. At the
...
\subsection{Post-freeze breadth and hardware planning}

An all-instance eligibility screen found eight eligible cases among 50 QOBLIB
MIS graphs under fixed exact-verification requirements. A separate Aer pilot
on the three newly eligible cases completed 18 exact references and 108 seeded
MPS jobs (54,000 shots), preserving all 24 candidate-versus-LR effect signs.
This extension is not pooled with the frozen two-backend replication and
does not provide three further rank-reversal examples.

A subsequent local planning study separates finite sampling from device
noise. With 10,000 shots per schedule, conservative simultaneous binomial
intervals resolve the ideal \texttt{football} comparison in only 53.43\% of
10,000 Monte Carlo replicates. Exact density-matrix simulation of the compiled
seven-qubit circuit on a synthetic line changes its effect from $+0.019269$
without noise to $-0.001387$ at two-qubit depolarizing parameter $10^{-3}$,
one-qubit parameter $10^{-4}$, and 1\% symmetric readout error. These are
planning results, not measurements on a QPU or a calibrated device model.
The supplementary information specifies the protocol and its limitations.
...
% Let the bibliography flow naturally; balancing from the second column
% caused an overfull final-page box in the expanded review draft.
