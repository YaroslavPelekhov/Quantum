from __future__ import annotations

import gc
import json
import time
from pathlib import Path

import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np

from qaoa_mis import (
    distribution_metrics,
    enumerate_exact_space,
    induced_subgraph,
    load_dimacs_graph,
    statevector_probabilities,
)
from run_multitask_normalized import paired_audit


ROOT = Path(__file__).resolve().parents[2]
INSTANCE_DIR = ROOT / "QOBLIB" / "07-independentset" / "instances"
SOURCE_PATH = Path(__file__).resolve().parent / "results_multitask_normalized" / "results.json"
OUTPUT_DIR = Path(__file__).resolve().parent / "results_random_subsets"
METHOD_SCALE = {
    "normalized_de": "max_coefficient",
    "unnormalized_de": "none",
    "normalized_random": "max_coefficient",
}
FROZEN_SPECS = (
    ("es60fst01", 4101, (8, 14, 17, 26, 28, 29, 40, 48, 50, 52, 53, 58, 60, 64, 76, 85, 105, 107, 118, 121)),
    ("es60fst01", 4102, (3, 8, 13, 17, 36, 37, 41, 56, 59, 62, 67, 73, 83, 88, 104, 105, 106, 111, 117, 118)),
    ("aves-sparrow-social", 5201, (1, 5, 6, 7, 9, 10, 11, 15, 17, 18, 23, 25, 27, 29, 34, 35, 38, 40, 43, 44, 46, 47)),
    ("aves-sparrow-social", 5202, (2, 3, 7, 8, 9, 13, 14, 16, 20, 21, 25, 26, 27, 28, 32, 34, 39, 42, 43, 44, 46, 49)),
    ("MANN-a9", 6301, (1, 4, 5, 7, 8, 9, 10, 11, 14, 15, 19, 20, 22, 24, 25, 27, 29, 30, 31, 34, 41, 42, 43, 45)),
    ("MANN-a9", 6302, (1, 2, 7, 8, 9, 10, 11, 12, 17, 18, 20, 21, 24, 26, 29, 32, 34, 36, 38, 39, 40, 43, 44, 45)),
)


def frozen_graphs():
    graphs = []
    for source_name, seed, one_based_vertices in FROZEN_SPECS:
        source = load_dimacs_graph(INSTANCE_DIR / f"{source_name}.gph")
        vertices = tuple(vertex - 1 for vertex in one_based_vertices)
        graphs.append(induced_subgraph(source, vertices, f"{source_name}_random_seed{seed}"))
    return graphs


def metric_audit(graphs_output, source, first, second, metric, seed):
    first_values = [
        np.mean([graph["methods"][first][replicate][metric] for graph in graphs_output])
        for replicate in range(len(source["replicates"]))
    ]
    second_values = [
        np.mean([graph["methods"][second][replicate][metric] for graph in graphs_output])
        for replicate in range(len(source["replicates"]))
    ]
    return paired_audit(first_values, second_values, seed)


def main() -> None:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    source = json.loads(SOURCE_PATH.read_text(encoding="utf-8"))
    graphs_output = []
    started = time.perf_counter()
    for graph in frozen_graphs():
        exact = enumerate_exact_space(graph)
        row = {
            "graph": {
                "name": graph.name,
                "vertices": graph.n,
                "edges": len(graph.edges),
                "exact_optimum": exact.optimum,
                "source": graph.source,
            },
            "methods": {},
        }
        for method, scale in METHOD_SCALE.items():
            values = []
            for replicate in source["replicates"]:
                params = np.asarray(replicate[method]["params"], dtype=float)
                probs = statevector_probabilities(graph, params, p=2, cost_scale=scale)
                values.append({"replicate": replicate["replicate"], **distribution_metrics(probs, exact)})
                del probs
                gc.collect()
            row["methods"][method] = values
        graphs_output.append(row)
        (OUTPUT_DIR / "checkpoint.json").write_text(json.dumps(graphs_output, indent=2), encoding="utf-8")
        del exact
        gc.collect()

    audits = {}
    for first, second, label in (
        ("normalized_de", "unnormalized_de", "primary_normalization"),
        ("normalized_de", "normalized_random", "secondary_evolution"),
    ):
        audits[label] = {
            metric: metric_audit(graphs_output, source, first, second, metric, 20261103 + offset)
            for offset, metric in enumerate(
                ("score", "approximation_ratio_unconditional", "optimal_probability", "feasible_probability")
            )
        }

    primary = audits["primary_normalization"]["score"]
    checks = {
        "primary_mean_positive": primary["difference"]["mean"] > 0,
        "primary_bootstrap_ci_positive": primary["difference"]["bootstrap_95_percent_ci"][0] > 0,
        "primary_paired_t_p_below_0_05": primary["paired_t_test_two_sided"]["p_value"] < 0.05,
        "primary_majority_wins": primary["first_wins"] > len(source["replicates"]) / 2,
    }
    report = {
        "experiment": "Pre-registered random-induced-subgraph robustness audit",
        "declaration": "Sources, vertices, seeds, schedules, methods, and endpoints were frozen in NOVELTY_AUDIT.md before evaluation.",
        "primary_endpoint": "Aggregate raw composite score: normalized DE minus identical unnormalized DE over six random induced graphs.",
        "secondary_endpoint": "Raw metric sensitivity and normalized DE minus equal-budget normalized random search.",
        "seconds": time.perf_counter() - started,
        "graphs": graphs_output,
        "audits": audits,
        "robustness_gate": {"checks": checks, "passed": all(checks.values())},
    }
    (OUTPUT_DIR / "results.json").write_text(json.dumps(report, indent=2), encoding="utf-8")

    labels = [row["graph"]["name"].replace("_random_", "\n") for row in graphs_output]
    x = np.arange(len(labels))
    width = 0.25
    plt.figure(figsize=(12.0, 5.2))
    for index, method in enumerate(METHOD_SCALE):
        samples = [[item["score"] for item in row["methods"][method]] for row in graphs_output]
        plt.bar(
            x + (index - 1) * width,
            [np.mean(sample) for sample in samples],
            width,
            yerr=[np.std(sample, ddof=1) for sample in samples],
            capsize=3,
            label=method.replace("_", " "),
        )
    plt.xticks(x, labels, fontsize=8)
    plt.ylabel("Exact raw composite score")
    plt.title("Random-induced-subgraph robustness")
    plt.grid(axis="y", alpha=0.2)
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "random_subset_scores.png", dpi=180)
    plt.close()

    print(json.dumps({
        "seconds": report["seconds"],
        "graphs": [row["graph"] for row in graphs_output],
        "primary": primary,
        "secondary": audits["secondary_evolution"]["score"],
        "gate": report["robustness_gate"],
        "results": str((OUTPUT_DIR / "results.json").resolve()),
    }, indent=2))


if __name__ == "__main__":
    main()
